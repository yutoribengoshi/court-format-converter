...
# 番号の下には文字が来ない。全角スペースの位置から本文が始まる。
# (left_chars, first_line_chars)
# 1行目 = left + firstLine の位置から
# 2行目以降 = left の位置から（= 番号＋スペースの直後）
BODY_INDENT = {
    0: (0, 1),   # 見出しなし直後 → 首行1字のみ
    1: (3, 1),   # 第１直下 → 第(0)+１(1)+　(2) → 本文は3字目、首行4字目
    2: (3, 1),   # １直下 → 　(1)+１(2)+　(3) → 本文は3字目、首行4字目
    3: (6, 1),   # (1)直下 → （(2)+１(3)+）(4)+　(5) → 本文は6字目、首行7字目
    4: (5, 1),   # ア直下 → 　(3)+ア(4)+　(5) → 本文は5字目、首行6字目
    5: (8, 1),   # (ｱ)直下 → （(4)+ｱ(5)+）(6)+　(7) → 本文は8字目、首行9字目
    6: (7, 1),   # ａ直下 → 　(5)+ａ(6)+　(7) → 本文は7字目、首行8字目
    7: (10, 1),  # (a)直下 → （(6)+a(7)+）(8)+　(9) → 本文は10字目、首行11字目
}
